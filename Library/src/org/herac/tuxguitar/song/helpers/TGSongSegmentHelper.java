...
public class TGSongSegmentHelper {
	...
	public void insertMeasures(TGSongSegment segment,int fromNumber,long move, int track){
		...
		while (it.hasNext()) {
			TGTrack currTrack = (TGTrack) it1.next();
	...
	public TGSongSegment createSegmentCopies( TGSongSegment srcSegment , int count ){
		...
		for( int i = 1 ; i < count ; i ++ ){
			for(int m = 0; m < mCount; m++ ){
				TGMeasureHeader header = ( (TGMeasureHeader)segment.getHeaders().get(m) ).clone(sm.getFactory() );
				...
				for( int t = 0 ; t < tCount ; t ++ ){
					...
					TGMeasure measure = ((TGMeasure)track.getMeasures().get( m )).clone(sm.getFactory(), header );
