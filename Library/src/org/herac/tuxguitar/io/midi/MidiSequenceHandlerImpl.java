...
public class MidiSequenceHandlerImpl extends MidiSequenceHandler{
	...
	private void init(){
        sequence = new MidiSequence(MidiSequence.PPQ,(int)TGDuration.QUARTER_TIME);
		for (int i = 0; i < getTracks(); i++) {
            sequence.addTrack(new MidiTrack());
		}
	}
	...
	public void addEvent(int track, MidiEvent event) {
		if(track >= 0 && track < sequence.countTracks()){
            sequence.getTrack(track).add(event);
		}
	}
    ...
    public void notifyFinish() {
		try {
            sequence.finish();
			new MidiFileWriter().write(sequence,1, stream);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
